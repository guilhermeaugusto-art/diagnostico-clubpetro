...
export function RadarChart({ state, width = 480 }: RadarChartProps): string {
  ...
  const dataPoints = BLOCK_ORDER.map((b, i) => {
    const a = angles[i];
  ...
  const dots = BLOCK_ORDER.map((b, i) => {
    ...
    const x = cx + r * v * Math.cos(a);
    const y = cy + r * v * Math.sin(a);
    return `<circle cx="${x.toFixed(2)}" cy="${y.toFixed(2)}" r="3.5" class="radar-dot" />`;
  }).join("");
  ...
  const labels = BLOCK_ORDER.map((b, i) => {
    const a = angles[i];
    ...
    return `
      <text x="${lx.toFixed(2)}" y="${ly.toFixed(2)}"
            ...
            class="radar-label" dy="${nameDy}">${BLOCKS[b].short}</text>
      <text x="${lx.toFixed(2)}" y="${ly.toFixed(2)}"
            text-anchor="${anchor}" dominant-baseline="${baseline}"
  ...
  return `
    <div class="radar-chart" aria-label="Gráfico de radar das seis frentes" role="img">
