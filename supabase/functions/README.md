...
| `diagnostico-esteira` (v3) | trigger em `participou_raiox` + cron a cada 10 min | Reconciliador com dedup por coluna e retry: conversões RD, evento OPPORTUNITY (OAuth de vm_app_keys, refresh automático) e Kommo (pipeline Fidelidade 8166623, tag `raiox-realizado`, dedup por e-mail/telefone). Log em `diagnostico_esteira_log`. |
